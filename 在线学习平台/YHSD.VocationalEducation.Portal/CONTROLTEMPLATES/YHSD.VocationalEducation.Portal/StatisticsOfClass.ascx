...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="StatisticsOfClass.ascx.cs" Inherits="YHSD.VocationalEducation.Portal.CONTROLTEMPLATES.YHSD.VocationalEducation.Portal.StatisticsOfClass" %>
...
<script type='text/javascript'>
    ...
    function BindDataToTab(data) {
        ...
        $(data).each(function (k) {
            var tr = "<tr class='" + GetTrClass(k) + "'><td class='td_tit'>" + GetTrNum(k, pageIndex) + "</td><td class='td_tit_left'>" + this.PaperName + "</td><td class='td_tit_left'>" + this.Class + "</td><td class='td_tit'>" + this.MaxScore + "</td><td class='td_tit'>" + this.AvgScore + "</td><td class='td_tit'>" + this.PassPercent + "</td></tr>";
            $("#tab tbody").append(tr);
        });
    ...
    function ShowData(index) {
        var postData = { CMD: "ExamToClass", PageSize: pageSize, PageIndex: index };
        var model = {};
    ...
    function ShowChartData() {
        var postData = { CMD: "ExamToClass", PageSize: 0, PageIndex: -1 };
        var model = {};
        var paperName = $("#<%=txtPaperName.ClientID%>").val().trim();
        if (paperName != "") {
            model.PaperName = paperName;
    ...
    $(function () {
        ...
        $("#<%=txtPaperName.ClientID%>,#<%=txtClassName.ClientID%>").keypress(function () {
            EnterEvent("btnQuery");
        })
...
<div class="TopToolbar">
    <span>试卷名称&nbsp</span><input type="text" runat="server" class="input_part" id="txtPaperName" style="width:150px;"/>&nbsp&nbsp
    <span>班级名称&nbsp</span><input type="text" runat="server" class="input_part" id="txtClassName" style="width:150px;"/>
    <input type="button" value="查询" class="button_s" id="btnQuery" />
    <asp:Button runat="server" Text="导出" ID="btnExport" CssClass="button_s" UseSubmitBehavior="false" onclick="btnExport_Click"/>
</div>
