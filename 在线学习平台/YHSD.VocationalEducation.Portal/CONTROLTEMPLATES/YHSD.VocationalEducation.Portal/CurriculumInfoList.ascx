...
<div>
    <asp:HiddenField ID="HidResourceNameID" runat="server" />
        ...
        data-options="url:'<%=YHSD.VocationalEducation.Portal.Code.Common.CommonUtil.GetChildWebUrl()%>/_layouts/15/YHSD.VocationalEducation.Portal/CurriculumInfoTree.aspx',required:false"></select>
                    &nbsp&nbsp
            <span>课程名称&nbsp</span>
            <asp:TextBox ID="CurriculumName" runat ="server" style="width:198px;height:22px;border:1px solid #dedede;"></asp:TextBox>
...
<%# Container.ItemIndex + 1 + (this.AspNetPageCurriculum.CurrentPageIndex -1)*this.AspNetPageCurriculum.PageSize %>
              ...
              <td class="td_tit_left"><a href="<%=YHSD.VocationalEducation.Portal.Code.Common.CommonUtil.GetChildWebUrl()%>/_layouts/15/YHSD.VocationalEducation.Portal/ChapterGuanLi.aspx?id=<%#Eval("Id")%>" target="_self">章节管理</a></td>

                         <td class="td_tit"><a title="编辑课程" href="<%=YHSD.VocationalEducation.Portal.Code.Common.CommonUtil.GetChildWebUrl()%>/_layouts/15/YHSD.VocationalEducation.Portal/CurriculumInfoNew.aspx?id=<%#Eval("Id")%>" target="_self"><img class="EditImg" /></a><a  onclick="DeleteKeChen('<%#Eval("Id") %>')" title="删除课程"><img class="DelImg" /></a></td>
                    </tr>
