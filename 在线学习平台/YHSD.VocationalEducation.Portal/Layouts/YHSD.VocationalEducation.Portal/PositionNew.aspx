...
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PositionNew.aspx.cs" Inherits="YHSD.VocationalEducation.Portal.Layouts.YHSD.VocationalEducation.Portal.PositionNew"  %>
        ...
        <script>
            function onSave() {
                ...
                if (document.getElementById("<%=this.Name.ClientID %>").value.replace(/(^\s*)|(\s*$)/g, "") == "") {
                    LayerTips("角色名称必须填写！", "<%=this.Name.ClientID %>");
                //layer.tips("角色名称必须填写！", "#" + "<%=this.Name.ClientID %>", { tips: [1, '#49B700'] });
                    return false;
                }

                if (document.getElementById("<%=this.Name.ClientID %>").value.length > 100) {
                    LayerTips("角色名称不能超过100个字符！", "<%=this.Name.ClientID %>");
                   // layer.tips("角色名称不能超过100个字符！", "#" + "<%=this.Name.ClientID %>", { tips: [1, '#00BC16'] });
                    return false;
                }
                if (document.getElementById("<%=this.PositionName.ClientID %>").value.replace(/(^\s*)|(\s*$)/g, "") == "") {
                    LayerTips("请给角色分配权限！", "PositionTree");
                    //layer.tips("请给角色分配权限！！", "#PositionTree", { tips: [1, '#00BC16'] });
                    return false;
                }
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#PositionTree").tree({
                ...
                animate: true,
                lines: true,
                url: '<%=YHSD.VocationalEducation.Portal.Code.Common.CommonUtil.GetChildWebUrl()%>/_layouts/15/YHSD.VocationalEducation.Portal/PositionJson.aspx',
