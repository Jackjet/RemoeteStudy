...
namespace YHSD.VocationalEducation.Portal.Layouts.YHSD.VocationalEducation.Portal
{
    public partial class ChapterNew : LayoutsPageBase
    {
        ...
        protected void BTSave_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Page.ClientScript.RegisterStartupScript(typeof(string), "ok", "<script>LayerAlert('添加章节成功!!',function(){window.location.href ='" + ViewState["retu"].ToString() + "';});</script>;");
            }
            else
            {
                this.Page.ClientScript.RegisterStartupScript(typeof(string), "ok", "<script>LayerAlert('添加章节成功!!',function(){window.history.go(-2);});</script>;");
            }
        }
