    ...
    public partial class Caurse : LayoutsPageBase
    {
            ...
            try
            {
                Privileges.Elevated((oSite, oWeb, args) =>
                {
                    using (new AllowUnsafeUpdates(oWeb))
                    {
                        ...
                        DataTable Student = user.GetStudentUserInfo_NO(Name).Tables[0];
        ...
        private string GetSubmitTask(string ExampaperID)
        {
            ...
            try
            {
                ...
                {
                    string Name = "评价作业";
                    if (item["Marker"].SafeToString() != "")
                    {
                        Name = "查看评价";
                    }
                    result += "<li><a>" + StuName(item["UserID"].SafeToString()) + "</a><div>" + item["Created"] + "</div><span><a style='width:100%;cursor:pointer;' onclick='CheckTask(" + item["ID"] + ")'>" + Name + "</a></span></li>";
                }
            }
            catch (Exception ex)
            {
        ...
        private DataTable BuildDataTable()
        {
            ...
            string[] arrs = new string[] { "Title", "MaxNum", "ApplyNum", "Attachments", "ID", "TeacherID", "TeacherXM", "TeacherZP", "Introduc", "WeekName", "AddressID" };
