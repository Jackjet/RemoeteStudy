...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TB_wp_ChoiceQuestionUserControl.ascx.cs" Inherits="SVDigitalCampus.Task_base.TB_wp_ChoiceQuestion.TB_wp_ChoiceQuestionUserControl" %>
...
<style>
    ...
    .b_Title{height:22px;line-height:22px;}
        ...
        <div class="Operation_area markover">
            ...
            <div class="right_add fr">
                ...
                <div class="Choose_TextBasket" runat="server" id="Basket">
                    <asp:Label ID="showtbcount" CssClass="showtbcount" runat="server" Text="0"></asp:Label><a href="#" onclick="ShowTextbasket();">试题篮</a>
                                    
                    <div class="textbaskt" id="Testbasket" style="display: none;">
                        <!--试题篮 数量-->
                        <h1>共<asp:Label ID="Totalcount" CssClass="question_num " runat="server" Text="0"></asp:Label>题,<asp:Label ID="TotalScore" Text="0" CssClass="question_nu " runat="server"></asp:Label>分
                                    <asp:HiddenField ID="TotalScoreVal" runat="server" Value="0"/></h1>
                        <div class="clear"></div>
                        <!--试题篮-->
                        <div class="qtype">
                            <ul class="qtype_num">
                                <li><span class="b_Type"> 标 题：</span><span class="b_Title"><asp:TextBox ID="Title" CssClass="b_Title" runat="server" Width="90px"></asp:TextBox></span></li>
                                <asp:ListView ID="lvEQBasket" runat="server" OnItemCommand="lvEQBasket_ItemCommand">
                                    <ItemTemplate>
                                        <li>
                                            ...
                                            <span class="b_Socre"><em>
                                                            <input id="txtScore" name="txtScore" onblur=<%#"CheckNumber(this,'"+ Eval("Type")+"');" %> class="num" onkeyup="this.value=this.value.replace(/\D/g,'1')" onafterpaste="this.value=this.value.replace(/\D/g,'1')" value='<%#Eval("Score") %>' runat="server" type="text" /></em></span>
                                            <span class="b_delete">
                        ...
                        <div class="basket_opteration">
                            <span class="basket_clear fl">
                                <asp:LinkButton ID="ClearAll" runat="server" OnClick="ClearAll_Click">清空全部</asp:LinkButton></span><span class="basket_submit fr">
                                    <asp:LinkButton ID="ToMakeExam" runat="server" OnClick="ToMakeExam_Click">完成</asp:LinkButton></span>
                        </div>
                    </div>
                </div>
                  <a href="#" onclick="LoadExamQ();" class="add">刷新加载试题</a>
              <%--  <asp:LinkButton ID="lbLoadExamQ" runat="server" CssClass="add" OnClick="lbLoadExamQ_Click">刷新加载试题</asp:LinkButton>--%>
            </div>
        </div>
        <div class="clear"></div>
                ...
                <div class="Display_form">
                    ...
                    <asp:ListView ID="lvExamQ" runat="server" OnPagePropertiesChanging="lvExamQ_PagePropertiesChanging" OnItemCommand="lvExamQ_ItemCommand" OnItemDataBound="lvExamQ_ItemDataBound">
                        <EmptyDataTemplate>
                            <table>
                                <tr>
                                    <td colspan="9" style="text-align: center">您好，暂无试题记录！
                                    </td>
                                </tr>
                            </table>
                        </EmptyDataTemplate>
                                ...
                                <table class="E_form">
                                    ...
                                    <tr>
                                        <td colspan="5" class="question"><span class="content"><%#Eval("Question") %></span><br />
                                            ...
                                            <%-- <span class="optiona"> <%#Eval("OptionA") %></span>&nbsp&nbsp&nbsp;<span class="optionb"><%#Eval("OptionB") %></span><br /><span  class="optiona"><%#Eval("OptionC") %></span>&nbsp&nbsp&nbsp;<span class="optionb"><%#Eval("OptionD") %></span><span  class="optiona"><%#Eval("OptionE") %></span>&nbsp&nbsp&nbsp;<span class="optionb"><%#Eval("OptionF") %></span>--%>
                                    ...
                                    <tr>
                                        <td colspan="5">
                                            ...
                                            <a href="#" class="fr" onclick="Look('<%#Eval("ID") %>','<%#Eval("TypeID") %>');">查看</a>&nbsp;<asp:Button ID="lbAdd" runat="server" Text="选择" CommandArgument='<%#Eval("ID")+"&"+Eval("TypeID")+"&"+Eval("QType")%>' CssClass='<%#Eval("choice") %>' CommandName="Add" /><%--<asp:LinkButton ID="" runat="server" CommandArgument='<%#Eval("ID")+"&"+Eval("TypeID")+"&"+Eval("QType")%>' CssClass="F_Choice"  CommandName="Add">加入试卷</asp:LinkButton>--%></td>
                                    </tr>
                                </table>
                            </div>
                            <div class="clear"></div>
...
<script type="text/javascript">
    ...
    function CheckNumber(obj, Typeid) {
        if ($(obj).val().length > 0) {
            if (isNaN($(obj).val())) {
                $(obj).val("0");
                return;
        ...
        $("input[name$='txtScore']").each(function () {
            totalscore = parseFloat(totalscore) + parseFloat($(obj).val());
        });
        $('#<%=TotalScore.ClientID%>').text(totalscore);
        $('#<%=TotalScoreVal.ClientID%>').val(totalscore);
    }
</script>
