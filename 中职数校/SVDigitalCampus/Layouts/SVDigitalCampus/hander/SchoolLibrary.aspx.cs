...
namespace SVDigitalCampus.Layouts.SVDigitalCampus.hander
{
    public partial class SchoolLibrary : LayoutsPageBase
    {
            ...
            try
            {
                Privileges.Elevated((oSite, oWeb, args) =>
                {
                    using (new AllowUnsafeUpdates(oWeb))
                    {
                        ...
                        string pid = Request.Form["GetMajorID"].ToString();
        ...
        private DataTable GetChildItem(string Pid)
        {
            ...
            {
                com.writeLogMessage(ex.Message, "MyDriveUserControl.GetChildItem");
            }
            return dtCatagory;
        }
        #endregion
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["NJ"].ToString() == Request.Form["MajorID"].ToString())
                    {
                        ...
                        for (int i = 0; i < sj.Length; i++)
                        {
                            ...
                            tr["ID"] = dr["NJ"].ToString() + sj[i].Split(',')[0].ToString();// +dr["ID"].ToString();
                        ...
                        if (termItem != null && url.IndexOf(termItem.Name) < 0)
                        {
                            ...
                            if (type == "copy")//复制
                            {
                                ...
                                if (url == "LibraryDoc")
                                {
                                    ...
                                    NewItem["SubJectID"] = Request.Form["hSubject"].ToString();
                                    NewItem["CatagoryID"] = Request.Form["hContent"].ToString();
                                    NewItem.Update();
                                }
                                else
                                {
                            ...
                            else//移动
                            {
                                ...
                                if (url == "LibraryDoc")
                                {
                                    termItem["SubJectID"] = Request.Form["hSubject"].ToString();
                                    termItem["CatagoryID"] = Request.Form["hContent"].ToString();
                                    termItem.Update();
                                }
                                else
                                {
