...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="EO_wp_AddBulletinUserControl.ascx.cs" Inherits="SVDigitalCampus.Executive_Office.EO_wp_AddBulletin.EO_wp_AddBulletinUserControl" %>
...
<script type="text/javascript">
  ...
  <div class="MenuDiv New_announcement">
      ...
      <div class="form_right">
       ...
       <div class="t_btn">          
          <input id="btnAdd" class="btn" onclick="Submit(this)" type="button" value="保存" />
       </div>
...
<script>
    ...
    function Submit(obj) {
        ...
        if (checkNull()) {
            obj.disabled = true;
             jQuery.ajax({
                 url: '<%=layoutstr%>' + "/SVDigitalCampus/BulletinHandler/BulletinHander.aspx?action=AddBulletin&" + Math.random(),   // 提交的页面
    ...
    function checkNull() {
        ...
        var txtContent = $('#<%=txtContent.ClientID%>').val();
        var ddlType = $('#<%=ddlType.ClientID%>').val();
        if (txtTitle.trim() == "" || txtContent.trim() == "" || ddlType.trim() == "" || Organization.trim() == "") {
            result = false;
        }
        return result;
    }
