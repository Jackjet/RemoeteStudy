...
namespace SVDigitalCampus.Examination_System.ES_wp_EditExamQuestion
{
    public partial class ES_wp_EditExamQuestionUserControl : UserControl
    {
        ...
        private void BindObjQuestion(int qid, SPWeb web, SPList list, string type, string Template)
        {
            try
            {
                if (list != null)
                {
                    ...
                    else if (Template.Contains("1") || Template.Contains("2"))
                    {
                        //选项
                    ...
                    //是否显示
                    if (item["IsShowAnalysis"].safeToString().Equals("1")) { 
                        rdoisshowN.Checked = false; rdoisshowY.Checked = true; }
                    else if (item["IsShowAnalysis"].safeToString().Equals("2"))
                    {
                        rdoisshowY.Checked = false;
                        rdoisshowN.Checked = true;

                    }
                    //解析
                    Analysis.InnerHtml = item["Analysis"] == null ? null : item["Analysis"].safeToString();
                    //是否显示解析在答题界面
                    if (item["Analysis"] == null || item["Analysis"].safeToString().Equals("2")) { rdoisshowN.Checked = true; } else { rdoisshowY.Checked = true; }
        ...
        private void BindSubject(SPListItem item)
        {
            ...
            {
                DataTable cdt = GetNodesBypid(Convert.ToInt32(mid+sid));
