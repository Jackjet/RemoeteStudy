...
namespace SVDigitalCampus.Examination_System.ES_wp_Examination
{
    public partial class ES_wp_ExaminationUserControl : UserControl
    {
        #region 定义页面变量
        //考试时间
        ...
        {
            if (Request["__EVENTTARGET"].safeToString().Equals("SubmitExam"))
            {
                SubmitExam();
            }
            if (!IsPostBack)
        ...
        private void BindQCount(DataTable examdb)
        {
            ...
            if (panduanpartdt.Rows.Count > 0)
            {
                ...
                {
                    answertwo.DataSource = onepartdt;
                    answertwo.DataBind();
                    //判断是否有多选题
                    if (morepartdt.Rows.Count > 0)
                    {
                        answerthree.DataSource = morepartdt;
                ...
                //判断是否有单选题
            else if (onepartdt.Rows.Count > 0)
            {
        ...
        public DataTable GetExamQuestion(string id)
        {
            ...
            string[] columns = { "Count", "ID", "ExamID", "Title", "TypeID", "qType", "Type", "Score", "Question", "OptionA", "OptionB", "OptionC", "OptionD", "OptionE", "OptionF", "OrderID", "kuangIsShow", "OptionAIsshow", "OptionBIsshow", "OptionCIsshow", "OptionDIsshow", "OptionEIsshow", "OptionFIsshow", "OneIsShow", "moreIsShow", "Analysis" };
            ...
            foreach (DataRow item in subdt.Rows)
            {
                ...
                //获取分析
                string isshowa = item["IsShowAnalysis"].safeToString();//1显示、2不显示
                if (isshowa.Equals("1")) { 
        ...
        private void SubmitExam()
        {
                ...
                Privileges.Elevated((oSite, oWeb, args) =>
            {
                using (new AllowUnsafeUpdates(oWeb))
                {
                    ...
                    if (list != null)
                    {
                        ...
                        examnewitem.Update();
                        SPList qlist = oWeb.Lists.TryGetList("试卷答题");
                        decimal qscore = 0;bool ismark = false;//是否需要阅卷
                        if (qlist != null)
                        {
                            int qcount = 0;
                            //循环录入试卷答题
                                ...
                                foreach (ListViewItem qitem in lv_ExamQ.Items)
                                {
                                    ...
                                    if (qID != null)
                                    {
                                        ...
                                        if (answer != null)
                                        {
                                            ...
                                            {
                                                //获取试题答案信息
                                                SPList olist = oWeb.Lists.TryGetList("试卷客观题");
                                                if (olist != null)
                                                {
                                            ...
                                            else if (Template.Value.Equals("4")) {
                                                ismark = true;
                                                continue;
                                            }
                                            eqnewitem.Update();
                        ...
                        if (examnewitem.ID > 0)
                        {
                            Page.ClientScript.RegisterStartupScript(this.GetType(), "mySript", "alert('试卷提交成功！');window.location.href='" + SietUrl + "ExamPaperList.aspx';", true);
                        }
                    }
                }
            }, true);
            ...
            catch (Exception ex)
            {
                ...
                log.writeLogMessage(ex.Message, "ES_wp_Examination.ascx提交试卷");
            }
        ...
        {
            AnswerBeginTime = DateTime.Now;//设置答题开始时间
            //this.examtr.Tick += examtr_Tick;////执行函数examtr_Tick

            //this.examtr.Enabled=true;
        }
        /// <summary>
        /// 考试时间计时
        /// </summary>
        ...
        /// <param name="e"></param>
        //protected void examtr_Tick(object sender, EventArgs e)
        //{
        //    int mint = Convert.ToInt32(Mins.Text);
        //    int scss = Convert.ToInt32(Seconds.Text);
        //    if (mint >= 0&&scss>0)
        //    {
        ...
        //            Mins.Text = mint.ToString() + "分";

        //            scss = 59;

        //        }

        //        Seconds.Text = scss.ToString() + "秒";

        //    }
        //    else
        //    {
        //        this.examtr.Enabled = false;
        //        this.Page.ClientScript.RegisterStartupScript(this.GetType(), "MyScript", "var c=confirm('答题时间结束，是否提交试卷！');if(c){<%IsSubmit=true;SubmitExam();%>}", true);
        //    }
        //}
    }
}
