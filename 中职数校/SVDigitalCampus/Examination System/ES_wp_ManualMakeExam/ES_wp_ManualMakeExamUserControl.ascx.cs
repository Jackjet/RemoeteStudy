...
using System.Xml;
namespace SVDigitalCampus.Examination_System.ES_wp_ManualMakeExam
{
    public partial class ES_wp_ManualMakeExamUserControl : UserControl
    {
        ...
        private void Bindtestbasket()
        {
            try
            {
                ...
                showEQBasketbt = CreateDataTableHandler.CreateDataTable(new string[] { "Type", "Count", "TypeID" });
                int count = 0;
            ...
            {
                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam.ascx_绑定试题蓝");
            }
        }
        /// <summary>
        /// 绑定专业
        ...
        protected void BindMajor()
        {
            ...
            {
                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam_绑定专业");
            }
        }
        /// <summary>
        /// 绑定试题类型
        ...
        private void BindddlExamQType()
        {
            ...
            catch (Exception ex)
            {
                ...
                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam_绑定试题类型");
            }
        }
        /// <summary>
        /// 获取绑定试题数据
        ...
        protected void BindListView(bool isused)
        {
            try
            {
                ...
                string[] columns = { "Count", "ID", "Title", "Major", "Subject", "Chapter", "Part", "Klpoint", "QType", "TypeID", "Type", "DifficultyShow", "Difficulty", "Author", "Created", "Status", "qStatus", "jStatus", "Question", "OptionA", "OptionB", "OptionC", "OptionD", "OptionE", "OptionF", "choice", "Isable", "OptionAIsshow", "OptionBIsshow", "OptionCIsshow", "OptionDIsshow", "OptionEIsshow", "OptionFIsshow", "MajorID", "SubjectID", "ChapterID", "KlpointID", "PartID", "IsShow" };
                        ...
                        if (Subject == null || Subject == "-1" || item["SubjectID"].safeToString().Equals(Subject))//学科
                        {
                            if (Chapter == null || Chapter == "-1" || item["ChapterID"].safeToString().Equals(Chapter))//章
                            {
                                if (Part == null || Part == "-1" || item["PartID"].safeToString().Equals(Part))//节
                                {
                                    ...
                                    {
                                        if (Difficulty == null ||Difficulty == "0"|| item["Difficulty"].safeToString().Contains(Difficulty))
                                        {
                                            if (Type == null || Type == "0" ||item["TypeID"].safeToString().Equals(Type))
                                            {
                                                ...
                                                newdr["Count"] = Count;
                                                newdr["Question"] = newdr["Question"].safeToString().Length > 200 ? (newdr["Question"].safeToString().Substring(0, 200) + "...") : newdr["Question"].safeToString();
                        ...
                        if (Chapter == null || Chapter == "-1" || item["ChapterID"].ToString().Equals(Chapter))//章
                        {
                            if (Part == null || Part == "-1" || item["PartID"].ToString().Equals(Part))//节
                            {
                                if (Klpoint == null || Klpoint == "-1" || item["KlpointID"].ToString().Equals(Klpoint))//知识点
                                {
                                    if (Type == null || Type == "0" || item["TypeID"].safeToString().Equals(Type))
                                    {
                                        if (Difficulty == null || Difficulty == "0" || item["Difficulty"].ToString().Equals(Difficulty))
                                        {
                                            Count++;
                                        }
                                    }
        ...
        protected void lvExamQ_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName.Equals("Add"))
                {
                    ...
                    foreach (DataRow item in listdb.Rows)
                    {
                        if (item["ID"].safeToString().Equals(ID.safeToString()) && item["QType"].safeToString().Equals(QType.safeToString()))
                        {
                            item["choice"] = "F_noChoice";
                            item["Isable"] = "disabled";
                        }
                    }
                    SaveXmlFromDT(listdb);
            ...
            {
                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam_试题加入到试题篮");
            }
        }
        /// <summary>
        /// 试题类型查询
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btnNoopsyche_Click(object sender, EventArgs e)
        {
            Response.Redirect("IntelligenceMakeExam.aspx");
        }
        ...
        protected void ToMakeExam_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (count.Equals("0"))
                {
                    this.Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "myScript", "alert('请选择要组卷的试题！');", true);
                    return;
                }
                ...
                if (!string.IsNullOrEmpty(Major) && Major != "-1")//选择专业的情况
                {
                    int Number = 0;
                ...
                Session["action"] = "ManualMakeExam&手动选题";
                Response.Redirect("MakeExamination.aspx");
            }
            catch (Exception ex)
            {

                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam_组卷界面跳转");
            }
        }
        /// <summary>
        /// 清空试题篮
        ...
        protected void ClearAll_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam_清空试题蓝");
            }
        }
        ...
        protected void lvEQBasket_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(iditem))
                {
                    ...
                    if (e.CommandName.Equals("del"))
                    {
                        ...
                        {
                            DataTable Testbasket = Session["Testbasket"] as DataTable;
                            DataTable newtestbasket = CreateDataTableHandler.CreateDataTable(new string[] { "ID", "Type", "QType" });
                            foreach (DataRow item in Testbasket.Rows)
                            {
                            ...
                            Session["Testbasket"] = newtestbasket;
            ...
            catch (Exception ex)
            {
                ...
                log.writeLogMessage(ex.Message, "ES_wp_ManualMakeExam_删除试题篮的试题");
            }
        }
