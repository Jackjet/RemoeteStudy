...
namespace SVDigitalCampus.Examination_System.ES_wp_ExamList
{
    public partial class ES_wp_ExamListUserControl : UserControl
    {
        ...
        private void BindExamList()
        {
            ...
            {
                UserPhoto user = new UserPhoto();
                //拼接考试查询条件（试卷名称+学生+1已提交+2已阅卷的考试信息）
                ...
                string examquerystr = CAML.And(
                    CAML.Eq(CAML.FieldRef("Status"), CAML.Value("2")),
                ...
                foreach (DataRow item in examalldt.Rows)
                {
                    ...
                    if (Major == null || Major == "0" || Major == "-1" || exampitemdt["MajorID"].ToString().Equals(Major))//专业
                    {
                        if (Subject == null || Subject == "0" || Subject == "-1" || exampitemdt["SubjectID"].ToString().Equals(Subject))//学科
                        {
                            if (ExamPaper == null || ExamPaper == "" || exampitemdt["ID"].ToString().Equals(ExamPaper))
                            {
                                ...
                                if (ClassID == null || ClassID == "0" || classid == ClassID)
                                {
                                    ...
                                    //string classidnum = exampitemdt["ClassID"].ToString().Replace(",", "");
                                    // DataTable classdb = user.GetClassNameByID(classidnum);//获取班级名称
                                    //if (classdb.Rows.Count == 0)
                                    //{
                                    //}
                                    //else { classname = classdb.Rows[0]["BJ"].safeToString(); }
            ...
            catch (Exception ex)
            {
                ...
                log.writeLogMessage(ex.Message, "ES_wp_ExamList_考试成绩列表绑定");
            }
        }

        //protected void btnExamP_Click(object sender, EventArgs e)
        //{
