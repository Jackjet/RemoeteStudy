    ...
    public partial class HomePage1 : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                BindCurriculumInfo();
                BindAllCurriculumInfo();
                BindPartCurriculumInfo();
                BindBanjiTongZhi(UserID);
            }
        }
        public void BindAllCurriculumInfo() {
            CurriculumInfo Curriculum = new CurriculumInfo();
            Curriculum.IsOpenCourses = "1";
            List<CurriculumInfo> ListCurr = new CurriculumInfoManager().Find(null);

            Repeater2.DataSource = ListCurr;
            Repeater2.DataBind();
        }
        /// <summary>
        /// 可注册课程
        /// </summary>
         public void BindPartCurriculumInfo() {
            CurriculumInfo Curriculum = new CurriculumInfo();
            Curriculum.IsOpenCourses = "1";

            List<CurriculumInfo> ListCurr = new CurriculumInfoManager().FindUserKe(CommonUtil.GetSPADUserID().Id);

            Repeater3.DataSource = ListCurr;
            Repeater3.DataBind();
        }
        //公开课
