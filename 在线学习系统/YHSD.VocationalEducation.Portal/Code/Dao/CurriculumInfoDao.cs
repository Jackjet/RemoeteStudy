...
namespace YHSD.VocationalEducation.Portal.Code.Dao
{
    public class CurriculumInfoDao
    {
        ...
        {
            string sql = "INSERT INTO CurriculumInfo (Id,Title,Description,ImgUrl,ResourceID,CreaterUserID,CreaterTime,ClickNumber,IsDelete,IsOpenCourses,IfFree,ClassPrice,StartTime,EndTime,ClassMode) "
            + " VALUES(@Id,@Title,@Description,@ImgUrl,@ResourceID,@CreaterUserID,@CreaterTime,@ClickNumber,@IsDelete,@IsOpenCourses,@IfFree,@ClassPrice,@StartTime,@EndTime,@ClassMode)";
            ...
            if (!String.IsNullOrEmpty(entity.IfFree))
                cmd.Parameters.Add("IfFree", SqlDbType.NVarChar).Value = entity.IfFree;
            else
                cmd.Parameters.Add("IfFree", SqlDbType.NVarChar).Value = DBNull.Value;
            if (!String.IsNullOrEmpty(entity.ClassPrice))
                cmd.Parameters.Add("ClassPrice", SqlDbType.NVarChar).Value = entity.ClassPrice;
            else
                cmd.Parameters.Add("ClassPrice", SqlDbType.NVarChar).Value = DBNull.Value;
            if (entity.StartTime!=null)
                cmd.Parameters.Add("StartTime", SqlDbType.NVarChar).Value = entity.StartTime;
            else
                cmd.Parameters.Add("StartTime", SqlDbType.NVarChar).Value = DBNull.Value;
            if (entity.EndTime != null)
                cmd.Parameters.Add("EndTime", SqlDbType.NVarChar).Value = entity.EndTime;
            else
                cmd.Parameters.Add("EndTime", SqlDbType.NVarChar).Value = DBNull.Value;
            if (entity.ClassMode != null)
                cmd.Parameters.Add("ClassMode", SqlDbType.NVarChar).Value = entity.ClassMode;
            else
                cmd.Parameters.Add("ClassMode", SqlDbType.NVarChar).Value = DBNull.Value;
            try
            {
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        public CurriculumInfo Get(String id)
        {
            ...
            string sql = "select Id,Title,Description,CreaterUserID,ImgUrl,ResourceID,CreaterTime,ClickNumber,IsDelete,IsOpenCourses,IfFree,ClassPrice from CurriculumInfo where Id = @Id";
            SqlConnection conn = ConnectionManager.GetConnection();
            SqlCommand cmd = new SqlCommand(sql, conn);
            ...
            try
            {
                ...
                while (rd.Read())
                {
                    ...
                    entity.IfFree = MySqlDataReader.GetString(rd, "IfFree");
                    entity.ClassPrice = MySqlDataReader.GetString(rd, "ClassPrice");
                }
            }
            catch (Exception)
            {
        ...
        {
            string sql = "UPDATE CurriculumInfo SET Id =@Id,Title =@Title,Description =@Description,ImgUrl =@ImgUrl,ResourceID =@ResourceID,CreaterTime =@CreaterTime,ClickNumber =@ClickNumber,IsDelete =@IsDelete,IsOpenCourses=@IsOpenCourses,IfFree=@IfFree,ClassPrice=@ClassPrice,StartTime=@StartTime,EndTime=@EndTime,ClassMode=@ClassMode where Id = @Id";
            SqlConnection conn = ConnectionManager.GetConnection();
            SqlCommand cmd = new SqlCommand(sql, conn);
            if (!String.IsNullOrEmpty(entity.Id))
                cmd.Parameters.Add("Id", SqlDbType.NVarChar).Value = entity.Id;
        ...
        public List<CurriculumInfo> Find(CurriculumInfo entity)
        {
            ...
            StringBuilder sql = new StringBuilder("select IfFree,ClassPrice, Id,Title,Description,ImgUrl,ResourceID,(select Name from ResourceClassification where Id=CurriculumInfo.ResourceID) as ResourceName ,CreaterUserID,CreaterTime,ClickNumber,IsDelete from CurriculumInfo where 1=1 and IsDelete=0 ");
            if (entity != null)
            {
                if (!string.IsNullOrEmpty(entity.Id))
                {
    ...
    on c.Id = d.Pid 
        ...
        {
            List<CurriculumInfo> list = new List<CurriculumInfo>();
            StringBuilder sql = new StringBuilder("select top 6 Id,Title,Description,ImgUrl,ResourceID,(select Name from ResourceClassification where Id=CurriculumInfo.ResourceID) as ResourceName ,CreaterUserID,CreaterTime,ClickNumber,IsDelete,(select Min(CreateTime) from ClassCurriculum where CurriculumID=CurriculumInfo.Id and ClassID=(select Cid from ClassUser where uid=@UserID)) as KaiKeTime from CurriculumInfo where IsDelete=0 and id not in(select CurriculumID from ClassCurriculum where ClassID=(select CId from ClassUser where Uid=@UserID)) and IsOpenCourses=0 order by KaiKeTime desc");
            SqlConnection conn = ConnectionManager.GetConnection();
            SqlCommand cmd = new SqlCommand(sql.ToString(), conn);
            cmd.Parameters.Add("@UserID", SqlDbType.NVarChar).Value = UserID;
            SqlDataReader rd = null;
