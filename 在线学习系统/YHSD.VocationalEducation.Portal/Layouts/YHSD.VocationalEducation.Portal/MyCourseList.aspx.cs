    ...
    public partial class MyCourseList : LayoutsPageBase
    {
        ...
        public void BindCurriculumInfo()
        {
            ...
            for (int i = 0; i < ListCurr.Count; i++)
            {
                ...
                if (ListCurr[i].StartTime != null && now_time < ListCurr[i].StartTime) { ListCurr.Remove(ListCurr[i]); ; }
                if (ListCurr[i].EndTime != null && now_time > ListCurr[i].EndTime) { ListCurr.Remove(ListCurr[i]); ; }

                if(ListCurr[i].IfFree=="0"){ListCurr[i].IfFree="免费";}
                else { ListCurr[i].IfFree = "收费"; }
