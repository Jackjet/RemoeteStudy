...
<asp:Content ID="PageHead" ContentPlaceHolderID="PlaceHolderAdditionalPageHead" runat="server">
    ...
    <script type="text/javascript">
        ...
        function AddSuiJiZuJuanGroup(name, timushuliang) {
            var html = "<div class='group'>\
                    <div class='title'>\
                        <div class='groupName'>" + name + "</div>\
                        <div class='groupTool'><a href='javascript:;' class='easyui-linkbutton' data-options='plain:true' onclick='EditGroup(this)'>修改分组</a>\
        ...
        $(function () {
            ...
            //弹出添加随机分组的页面
            $("#SuiJiFenZu").click(function () {

                OL_ShowLayer(1, '添加分组', 335, 170, $('#addSuiJiZuJuanGroupDialog'), function () {
                    $("#SuiJiZuJuantxtGroupName").val("");
                    EditGroupText = undefined;
                });
                $("#SuiJiZuJuantxtGroupName").focus();
            })
            //添加分组
            ...
            //添加随机分组
            $("#SuiJiZuJuanbtnEnterGroup").click(function () {
                var name = $("#SuiJiZuJuantxtGroupName").val().trim();
                var timushuliang = $("#timushuliang").val().trim();
              
                if (name == "" || timushuliang=="") {
                    LayerAlert("请输入分组名称和题目数量！");
                    return;
                }
                if (EditGroupText != undefined) {
                    $(EditGroupText).text(name);
                ...
                AddSuiJiZuJuanGroup(name,timushuliang);
                OL_CloseLayer();
            })
        ...
        function ShowSuiJiZuQuan(count)
        {
            //alert(1)
            var postData = { CMD: "FullTab", TypeName: "QuestionStore", PageSize: count, PageIndex: 1 };
            var model = {};
            ...
            AjaxRequest("Handler/CommonHandler.aspx", postData, function (returnVal) {
                PagerRefresh(1, returnVal.PageCount);
                //alert(returnVal.Data)
                BindDataToTabSuiJiZuQuan($.parseJSON(returnVal.Data));
            })
        }
        function BindDataToTabSuiJiZuQuan(bindData) {
            $(".questionDiv").empty();
            $(bindData).each(function () {
                var html = "<div class='questionItem' qid='" + this.ID + "'>\
                                            <div class='content'>\
                                                <span class='qType'> (" + this.QuestionType + ") </span><span>" + this.Title + "</span>\
                                            </div>\
                                            <div class='buttomTool'>\
                                                <span>分值&nbsp</span><input class='easyui-numberbox txtScore' data-options='min:1,width:40'>\
                                                <div class='DelImg' onclick='delQuestion(\"" + this.ID + "\");' title='删除'></div>\
                                                <div class='TopImg' onclick='Up(this)' title='上移'></div>\
                                                <div class='BottomImg' onclick='Buttom(this)' title='下移'></div>\
                                            </div>\
                                        </div>";
...
<asp:Content ID="Main" ContentPlaceHolderID="PlaceHolderMain" runat="server">
        ...
        <table class="tableEdit">
            ...
            <tr>
                <th class="td1">题目&nbsp;<span style="color: Red">*</span></th><td> 已有<span id="questionCount">0</span>道题
                    <input type="button" id="btnAddGroup" value="添加分组" class="button_s" /><input type="button" id="SuiJiFenZu" value="随机组卷" class="button_s" /></td>
            </tr>
            <tr>
    ...
    <div class="hiddenDiv">
        ...
        <div id="addSuiJiZuJuanGroupDialog" style="padding:10px; display:none;text-align:center;"><br />
            <span>名称&nbsp;<span style="color: Red">*</span>
            <input type="text" class="input_part" id="SuiJiZuJuantxtGroupName" style="width:240px;"/></span><br /><br />
            <span>题目数量&nbsp;<span style="color: Red">*</span>
            <input type="text" class="input_part" id="timushuliang" style="width:240px;"/></span><br /><br />
            <input type="button" value="确定" id="SuiJiZuJuanbtnEnterGroup" onclick="" class="button_s" /><input type="button" value="取消" id="SuiJiZuJuanbtnCancelGroup" onclick="                    OL_CloseLayer()" class="button_n" />
        </div>
    </div>
</asp:Content>
