...
namespace UserCenterSystem
{
    ...
    public partial class TeaStuList : BaseInfo //System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //加载列表数据
                //BindList("");
                BindStudent(GetStrWhere());
                BindTeacher(GetStrWhere());
            }
        }
        /// <summary>
        /// 绑定列表
        ...
        /// </summary>
        /// <param name="where"></param>
        private void BindStudent(string where)
        {
            try
            {
                Base_GradeBLL bll = new Base_GradeBLL();
                lvPeriod.DataSource = bll.Select("*","Base_Student",where);
                lvPeriod.DataBind();
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="where"></param>
        private void BindTeacher(string where)
        {
            try
            {
                Base_GradeBLL bll = new Base_GradeBLL();
                ListView1.DataSource = bll.Select("*", "Base_Teacher", where);
                ListView1.DataBind();
            }
            catch (Exception ex)
            {
        ...
        private string GetStrWhere()
        {
            ...
            //班级
            if (ddlBJ.SelectedValue != "全部")
            {
                sb.Append(" and BH='" + ddlBJ.SelectedValue + "' ");
            }
            return sb.ToString();
        }
        ...
        {
            Base_GradeBLL bll = new Base_GradeBLL();
            DataTable dt=bll.Select("BJBH,BJ", "Base_Class", "");
            ddlBJ.DataValueField = "BJBH";
            ddlBJ.DataTextField = "BJ";
            ddlBJ.DataSource = dt;
            ddlBJ.DataBind();
            ddlBJ.Items.Insert(0, new ListItem("全部", "全部"));
        }

        protected void ListView1_PagePropertiesChanging(object sender, PagePropertiesChangingEventArgs e)
        {
            DataPager1.SetPageProperties(DataPager1.StartRowIndex, DataPager1.MaximumRows, false);
            BindList(GetStrWhere());
        }
